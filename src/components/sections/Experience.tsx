      ...
      <div className="max-w-4xl mx-auto">
        <div className="relative border-l-2 border-zinc-200 dark:border-zinc-800 ml-3 md:ml-6 space-y-12">
          {experiences.map((exp, index) => (
            ...
            >
              {/* Timeline Dot */}
              <span className="absolute -left-[9px] top-6 h-5 w-5 rounded-full bg-indigo-600 ring-4 ring-white dark:ring-black transition-transform hover:scale-125" />

              <Card className="bg-zinc-100 dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 transition-all duration-300 hover:border-indigo-500/50 hover:shadow-[0_0_20px_rgba(99,102,241,0.15)]">
                ...
                <CardContent>
                  <ul className="space-y-3 mt-2">
                    {exp.description.map((point, i) => (
                      <li
                        key={i}
                        className="flex items-start gap-3 text-zinc-600 dark:text-zinc-300 leading-relaxed"
                      >
                        <span className="mt-2 h-1.5 w-1.5 min-w-[6px] rounded-full bg-indigo-500/80" />
                        <span>{point}</span>
                      </li>
                    ))}
                  </ul>
